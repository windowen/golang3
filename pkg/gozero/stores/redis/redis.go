...
import (
	...
	"serverApi/pkg/gozero/breaker"
	errorx2 "serverApi/pkg/gozero/errorx"
	"serverApi/pkg/gozero/mapping"
...
func NewRedis(conf RedisConf, opts ...Option) (*Redis, error) {
	...
	if !conf.NonBlock {
		if err := rds.checkConnection(conf.PingTimeout); err != nil {
			return nil, errorx2.Wrap(err, fmt.Sprintf("redis connect error, addr: %s", conf.Host))
		}
	}
...
func acceptable(err error) bool {
	return err == nil || errorx2.In(err, red.Nil, context.Canceled)
}
