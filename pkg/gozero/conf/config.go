...
import (
	...
	"serverApi/pkg/gozero/encoding"
	"serverApi/pkg/gozero/jsonx"
	mapping2 "serverApi/pkg/gozero/mapping"
)
...
var (
	fillDefaultUnmarshaler = mapping2.NewUnmarshaler(jsonTagKey, mapping2.WithDefault())
...
func LoadFromJsonBytes(content []byte, v any) error {
	...
	return mapping2.UnmarshalJsonMap(lowerCaseKeyMap, v, mapping2.WithCanonicalKeyFunc(toLowerCase))
}
...
func buildAnonymousFieldInfo(info *fieldInfo, lowerCaseName string, ft reflect.Type, fullName string) error {
	...
	case reflect.Map:
		elemField, err := buildFieldsInfo(mapping2.Deref(ft.Elem()), fullName)
		if err != nil {
			return err
		}
...
func buildFieldsInfo(tp reflect.Type, fullName string) (*fieldInfo, error) {
	tp = mapping2.Deref(tp)

	switch tp.Kind() {
	case reflect.Struct:
		return buildStructFieldsInfo(tp, fullName)
	case reflect.Array, reflect.Slice:
		return buildFieldsInfo(mapping2.Deref(tp.Elem()), fullName)
...
func buildNamedFieldInfo(info *fieldInfo, lowerCaseName string, ft reflect.Type, fullName string) error {
	...
	case reflect.Map:
		elemInfo, err := buildFieldsInfo(mapping2.Deref(ft.Elem()), fullName)
		if err != nil {
			return err
		}
