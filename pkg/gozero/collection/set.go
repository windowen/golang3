...
func (s *Set) validate(i any) {
	...
	case int:
		if s.tp != intType {
			zlogger.Errorf("element is int, but set contains elements with type %d", s.tp)
		}
	case int64:
		if s.tp != int64Type {
			zlogger.Errorf("element is int64, but set contains elements with type %d", s.tp)
		}
	case uint:
		if s.tp != uintType {
			zlogger.Errorf("element is uint, but set contains elements with type %d", s.tp)
		}
	case uint64:
		if s.tp != uint64Type {
			zlogger.Errorf("element is uint64, but set contains elements with type %d", s.tp)
		}
	case string:
		if s.tp != stringType {
			zlogger.Errorf("element is string, but set contains elements with type %d", s.tp)
		}
	}
}
