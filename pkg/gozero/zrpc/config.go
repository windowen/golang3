...
import (
	...
	discov2 "serverApi/pkg/gozero/discov"
	"serverApi/pkg/gozero/service"

	"serverApi/pkg/gozero/stores/redis"

	"serverApi/pkg/gozero/zrpc/internal"
	"serverApi/pkg/gozero/zrpc/resolver"
)
...
type (
	...
	RpcClientConf struct {
		Etcd          discov2.EtcdConf `json:",optional,inherit"`
...
func (cc RpcClientConf) BuildTarget() (string, error) {
	...
	if cc.Etcd.HasAccount() {
		discov2.RegisterAccount(cc.Etcd.Hosts, cc.Etcd.User, cc.Etcd.Pass)
	}
	if cc.Etcd.HasTLS() {
		if err := discov2.RegisterTLS(cc.Etcd.Hosts, cc.Etcd.CertFile, cc.Etcd.CertKeyFile,
