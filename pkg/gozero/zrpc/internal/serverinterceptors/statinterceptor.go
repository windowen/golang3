...
import (
	...
	"serverApi/pkg/gozero/timex"

	"serverApi/pkg/gozero/collection"
	"serverApi/pkg/gozero/lang"
	stat2 "serverApi/pkg/gozero/stat"
...
func UnaryStatInterceptor(metrics *stat2.Metrics, conf StatConf) grpc.UnaryServerInterceptor {
...
func logDuration(ctx context.Context, method string, req any, duration time.Duration,
	...
	if !shouldLogContent(method, ignoreMethods) {
		if isSlow(duration, durationThreshold) {
			zlogger.Infof("[RPC] slowcall - %s - %s", addr, method)
		}
	} else {
		content, err := json.Marshal(req)
		if err != nil {
			zlogger.Errorf("%s - %s", addr, err.Error())
		} else if duration > slowThreshold.Load() {
			zlogger.Infof("[RPC] slowcall - %s - %s - %s", addr, method, string(content))
		} else {
			zlogger.Infof("%s - %s - %s", addr, method, string(content))
		}
	}
}
