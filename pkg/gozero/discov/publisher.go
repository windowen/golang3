...
import (
	...
	"serverApi/pkg/gozero/proc"

	internal2 "serverApi/pkg/gozero/discov/internal"
	"serverApi/pkg/gozero/lang"
	"serverApi/pkg/gozero/syncx"
	"serverApi/pkg/gozero/threading"
...
func (p *Publisher) doKeepAlive() error {
	...
	for range ticker.C {
		...
		default:
			cli, err := p.doRegister()
			if err != nil {
				zlogger.Errorf("etcd publisher doRegister: %s", err.Error())
				break
			}

			if err := p.keepAliveAsync(cli); err != nil {
				zlogger.Errorf("etcd publisher keepAliveAsync: %s", err.Error())
				break
			}
...
func (p *Publisher) doRegister() (internal2.EtcdClient, error) {
	cli, err := internal2.GetRegistry().GetConn(p.endpoints)
	if err != nil {
		return nil, err
	}
...
func (p *Publisher) keepAliveAsync(cli internal2.EtcdClient) error {
			...
			case _, ok := <-ch:
				if !ok {
					...
					if err := p.doKeepAlive(); err != nil {
						zlogger.Errorf("etcd publisher KeepAlive: %s", err.Error())
					}
					return
				}
			case <-p.pauseChan:
				zlogger.Infof("paused etcd renew, key: %s, value: %s", p.key, p.value)
				p.revoke(cli)
				select {
				case <-p.resumeChan:
					if err := p.doKeepAlive(); err != nil {
...
func (p *Publisher) register(client internal2.EtcdClient) (clientv3.LeaseID, error) {
...
func (p *Publisher) revoke(cli internal2.EtcdClient) {
	if _, err := cli.Revoke(cli.Ctx(), p.lease); err != nil {
		zlogger.Errorf("etcd publisher revoke: %s", err.Error())
	}
}
