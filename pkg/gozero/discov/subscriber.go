...
func NewSubscriber(endpoints []string, key string, opts ...SubOption) (*Subscriber, error) {
	...
	if err := internal2.GetRegistry().Monitor(endpoints, key, sub.items, sub.exactMatch); err != nil {
		return nil, err
	}
...
func (c *container) OnAdd(kv internal2.KV) {
	c.addKv(kv.Key, kv.Val)
	c.notifyChange()
}

func (c *container) OnDelete(kv internal2.KV) {
	c.removeKey(kv.Key)
	c.notifyChange()
}
