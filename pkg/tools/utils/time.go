...
// GetYMDHmSBillNo 生成 prefix 前缀 + 年月日时分秒 + 后缀的订单号
...
func GetBjTimeLoc() *time.Location {
	...
	if err != nil {
		log.Printf("加载 Asia/Shanghai 时区失败，使用固定时区：%v", err)
...
// PayInTradeNo 获取代收订单号
func PayInTradeNo() string {
	// 获取当前时间
	now := time.Now()
	...
	// 拼接时间字符串和随机数字
	finalStr := fmt.Sprintf("PI%s%04d", timeStr, randomNumber)
	return finalStr
}

// PayOutTradeNo 获取代付订单号
func PayOutTradeNo() string {
	// 获取当前时间
	now := time.Now()
	...
	// 拼接时间字符串和随机数字
	finalStr := fmt.Sprintf("PO%s%04d", timeStr, randomNumber)
	return finalStr
}
